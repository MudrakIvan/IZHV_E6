        ...
        { // Main Window Area
            ...
            GUILayout.BeginVertical();
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    ...
                    interactiveMode = GUILayout.Toggle(interactiveMode, " Interactive mode", GUILayout.Width(WINDOW_DIMENSION.x / 2.0f));
                    if (GUI.changed)
                    {
                        GameManager.Instance.interactiveMode = interactiveMode;
                    }
                    var mute = SoundManager.Instance.masterMuted;
                    mute = GUILayout.Toggle(mute, " Mute", GUILayout.Width(WINDOW_DIMENSION.x / 2.0f));
                    if (GUI.changed)
                    {
